...
# Chrome 124 on macOS — 真实浏览器的 UA。Redmine WAF 会拦截
# requests 默认的 "python-requests/x.y" UA,补上后接口恢复正常。
# 同时给 OP / Redmine 两端都加上,避免单点失败。
_BROWSER_USER_AGENT = (
...
def _get_headers() -> dict:
    """OpenProject 请求头(Bearer + 浏览器伪装头)"""
    return {
        "Authorization": f"Bearer {OPENPROJECT_KEY}",
        "Content-Type": "application/json",
        **_COMMON_BROWSER_HEADERS,
        # OpenProject 服务端不强校验 Referer/Origin,这里带上避免被通用 WAF 误判
        "Referer": f"{OPENPROJECT_URL}/",
        "Origin": OPENPROJECT_URL.rstrip("/"),
    }


def _get_redmine_headers() -> dict:
    """Redmine 请求头(X-Redmine-API-Key + 浏览器伪装头)"""
    return {
        "X-Redmine-API-Key": REDMINE_KEY,
        "Content-Type": "application/json",
        **_COMMON_BROWSER_HEADERS,
        # Redmine 公网入口被 WAF 守护,Referer/Origin 必带
        "Referer": f"{REDMINE_URL}/",
        "Origin": REDMINE_URL.rstrip("/"),
    }
